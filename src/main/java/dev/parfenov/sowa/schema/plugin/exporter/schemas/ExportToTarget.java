...
public class ExportToTarget implements SchemaExporter {
    ...
    public ExportToTarget(ExportConfig config) {
        this.config = config;
        this.mapper = new ObjectMapper();
    }

    @Override
    public void export(List<SowaSchema> sowaSchemaList) {
        if (CollectionUtils.isEmpty(sowaSchemaList)) {
            return;
        }
        ...
        try {
            exportJson(requestDir, responseDir, sowaSchemaList);
        } catch (IOException e) {
            throw new RuntimeException("Ошибка во время экспорта в .json: " + sowaSchemaList, e);
        }
    }
